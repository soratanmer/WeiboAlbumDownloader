...
namespace WeiboAlbumDownloader.Helpers
{
    ...
    public static class FfmpegInvoker
    {
        ...
        public static string? LocateFfmpeg()
        {
            ...
            {
                var bundled = Path.Combine(baseDir, OperatingSystem.IsWindows() ? "ffmpeg.exe" : "ffmpeg");
                if (File.Exists(bundled)) return bundled;
            }

            var cwdName = OperatingSystem.IsWindows() ? "ffmpeg.exe" : "ffmpeg";
            var cwd = Path.Combine(Environment.CurrentDirectory, cwdName);
            ...
            try
            {
                foreach (var dir in (Environment.GetEnvironmentVariable("PATH") ?? string.Empty).Split(Path.PathSeparator))
                {
                    ...
                    var hit = Path.Combine(dir.Trim(), cwdName);
        ...
        /// 视频流零转码拷贝，音频重编码为标准 AAC。
        /// 显式 <c>-map 0:v:0 / 0:a:0?</c> 只保留视频与音频轨，从而剔掉 iPhone 专有的 mebx 元数据轨；
        /// <c>-movflags +faststart</c> 使 moov 前置，提升 OPPO/小米等严格解析器的兼容性。
